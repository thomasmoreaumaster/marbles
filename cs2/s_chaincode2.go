...
var assoIndexStr = "_assoindex"       //name for the key/value that will store a list of all known marbles
...
type Asso struct {
	...
	Adherents   []string  `json:"adherents"` //user who created the open trade order
	Scrutins    []Scrutin `json:"scrutins"`
}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	} else if function == "init_asso" { //create a new marble
		return t.init_asso(stub, args)
...
// ============================================================================================================================
// Init Asso - create a new foundation, store into chaincode state, update indexes
// ============================================================================================================================
func (t *SimpleChaincode) init_asso(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	var err error
	// "nameAsso", "descriptionAsso", "Owner"
	if len(args) != 3 {
	...
	nameAsso := args[0]
	descriptionAsso := strings.ToLower(args[1])
	owner := strings.ToLower(args[2])
	var adherents []string
	var scrutins []Scrutin

	//check if scrutin already exists
	assoAsBytes, err := stub.GetState(nameAsso)
	if err != nil {
	...
	asso.Name = nameAsso
	asso.Description = descriptionAsso
	asso.Owner = owner
	asso.Adherents = adherents
	asso.Scrutins = scrutins

	jsonsAsBytes, _ := json.Marshal(asso)
	err = stub.PutState(nameAsso, jsonsAsBytes) //rewrite the marble with id as key
	if err != nil {
		return nil, err
	}

	//get the marble index
	assosAsBytes, err := stub.GetState(assoIndexStr)
	if err != nil {
		return nil, errors.New("Failed to get scrutin index")
	}
	var assoIndex []string
	json.Unmarshal(assosAsBytes, &assoIndex) //un stringify it aka JSON.parse()

	//append
	assoIndex = append(assoIndex, nameAsso) //add marble name to index list
	fmt.Println("! scrutin index: ", assoIndex)
	jsonAsBytes, _ := json.Marshal(assoIndex)
	err = stub.PutState(assoIndexStr, jsonAsBytes) //store name of marble

	fmt.Println("- end init asso")
	return nil, nil
}
